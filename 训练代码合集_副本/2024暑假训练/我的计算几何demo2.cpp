...
void solve(int cases) {
    ...
    for (int i = 0; i < (int)q.size(); i++) {
        int op = f.relation(q[i]);
        if (op == -1 || op == 0 || op == 4) continue;
        auto res = f.inter(q[i]);
        for (auto j : res) res1.push_back(j);
    }
    auto ans1 = convexhull(res1).p;
    for (int i = 0; i < (int)ans1.size(); i++) {
        for (int j = 0; j < (int)ans1.size(); j++) {
            ans = std::max(ans, ans1[i].dis(ans1[j]));
        }
    }

    // 第二部分，验证是否可以取到直径
    using arc_t = tuple<Point, long double, long double, long double>;  // 存放左右端点
    vector<vector<arc_t>> arcs(q.size());
    auto cut_circ = [&](const Circle &ci, const size_t i) {
        vector<pair<long double, int>> evt;
        evt.push_back({-PI, 0});
        evt.push_back({PI, 0});
    ...
    q.push_back(f);
    cut_circ(f, q.size() - 1);
    std::sort(arcs[0].begin(), arcs[0].end());
    int tag = 0;
    for (int i = 0; i < (int)arcs[0].size(); i++) {
        auto [u1, u2, u3, u4] = arcs[0][i];
        u3 += PI, u4 += PI;
        if (u3 > PI * 2 + eps) u3 -= PI * 2;
        if (u4 > PI * 2 + eps) u4 -= PI * 2;
        auto in_it = [](long double &a, arc_t &b) {
            auto [v1, v2, v3, v4] = b;
            if (v3 - eps < a && v4 + eps > a)
                return 1;
            else
                return 0;
        };
        for (int j = 0; j < (int)arcs[0].size(); j++) {
            if (in_it(u3, arcs[0][j])) {
                tag = 1;
                break;
            }
            if (in_it(u4, arcs[0][j])) {
                tag = 1;
                break;
            }
        }
    ...
    if (tag) ans = std::max(ans, f.r * 2);
    printf("Case #%d: %.15Lf", cases, ans);
}
