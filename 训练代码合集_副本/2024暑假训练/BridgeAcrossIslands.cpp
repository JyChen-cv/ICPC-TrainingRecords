...
template<typename T = ini>struct polygon {  // 普通多边形，点集// 记得开vector
	...
	void rotatingCaliper(const auto &func) {
		// 只要不是自己和自己操作都需要先找到最大值点开始旋转，不然没有办法保证单调性
		// 只要不是自己和自己操作都最好带上面积的绝对值
		if (p.size() == 1) return;
		if (p.size() == 2) {
			func(p[0], p[1], p[1]);
			return;
		}
		auto area = [](const Point & u, const Point & v, const Point & w) {return abs(cross((w - u) , (w - v)));};

		for (int i = 0, j = 0; i < (int)p.size(); i++) {
			int nxti = nxt(i);

			while (cmp(area(p[i], p[nxti], p[j]) , area(p[i], p[nxti], p[nxt(j)])) <= 1) {
				j = nxt(j);
	...
	T diameter2() {  // 不是完全意义上的直径，只是求平面最远点对吧，直径有可能不是点对间的距离
		T ans = 0;
		// auto应该是一个地址值
...
void caliper(Polygon &a, Polygon &b, auto func) {
	auto area = [](const Point & u, const Point & v, const Point & w) {return (cross((w - u) , (w - v)));};
	int j = 0;
	for (int i = 0; i < b.size(); i++) {
		if (cmp(area(a[0], a[1], b[i]) , area(a[0], a[1], b[j])) >= 0) j = i;
	}
	for (int i = 0; i < a.size(); i++) {
		int nxti = a.nxt(i);
		func(a[i], a[nxti], b[j]);
		if (cmp(area(a[i], a[nxti], b[j]) , area(a[i], a[nxti], b[b.nxt(j)] )) <= 0) {
			j = b.nxt(j);
			func(a[i], a[nxti], b[j]);
		}
		// std::cout << a[i].x << " " << a[i].y << " " << a[nxti].x << " " << a[nxti].y << " " << b[j].x << " " << b[j].y << std::endl;
	}
}
...
bool solve() {
	...
	Polygon aa = a.convexHull();
	Polygon bb = b.convexHull();
	db ans = 1e15;
	auto func = [&](const Point & u, const Point & v, const Point & w) {
		ans = min(ans, (w - u).abs());
		ans = min(ans, (w - v).abs());
		Point ls = projection(w, Line{u, v});
		if (onSegement(ls, Line({u, v}))) ans = std::min(ans, (w - ls).abs());
		// std::cout << ans << std::endl;
	};
	caliper(aa, bb, func), caliper(bb, aa, func);
