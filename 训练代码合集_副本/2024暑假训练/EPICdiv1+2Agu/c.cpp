...
#define P0 (Point) {0,0}
typedef long double db;  // 开long double记得改输入输出函数
...
void solve() {
	...
	int tag = 1;
	for (int i = 0; i < n; i++) {
		if (cmp(dis(q[i], b), t) <= 0) tag = 0;
	}
	// for (int i = 0; i < n; i++) {
	// 	Point c = reflection(q[i], Line{a, b});
	// 	if (cmp(c.x, a.x)*cmp(c.x,b.x)<=0) {
	// 		if (cmp(dis(c, q[i]),t)<=0) tag=0;
	// 	}
	// 	else {
	// 		if (cmp(c.x,a.x)<=0) {
	// 			if (cmp(dis(q[i],b),q))
