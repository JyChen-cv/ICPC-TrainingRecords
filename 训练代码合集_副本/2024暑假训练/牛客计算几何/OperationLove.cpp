...
int pointPolygonPosCheck(point k1, polygon k2) {
	...
	for (int i = 0; i < num; i++) {if (onSegement(k1, (line) {k2[i], k2[(i + 1) % num]})) {return 1;}}
	for (int i = 0; i < num; i++) {
		db lmin = min(k2[i].y, k2[(i + 1) % num].y), lmax = max(k2[i].y, k2[(i + 1) % num].y);
		if (lmin > k1.y || lmax < k1.y || lmin == lmax || lmin == k1.y) continue;
...
int main() {
	...
	while (t--) {
		...
		int num = p.size();
		for (int i = 0; i < num; i++) {
			// std::cout << cmp(dis(p[i], p[(i + 1) % num]) , 9) << std::endl;
			if (cmp(dis(p[i], p[(i + 1) % num]) , 9) == 0) {
				if (cmp(dis(p[(i - 1 + num) % num], p[i]) , 6) == 0) {
					if (toLeft(p[(i - 1 + num) % num], (line) {p[i], p[(i + 1) % num]}) == 1) std::cout << "right" << std::endl;
					else std::cout << "left" << std::endl;
				}
				else {
					if (toLeft(p[(i - 1 + num) % num], (line) {p[i], p[(i + 1) % num]}) == 1) std::cout << "left" << std::endl;
