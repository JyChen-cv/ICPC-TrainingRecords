...
struct point {
	...
	point cturn90() {return (point) {y, -x};}  // 顺时针旋转90度
	// 逆时针旋转
	point acturn(db k) {return (point) {x*cos(k) - y*sin(k), x*sin(k) + y*cos(k)};}
	point acturn90() {return (point) { -y, x};}  // 逆时针旋转90度
	// 返回模长
...
polygon convexHull(vector<point> &k) {
	sort(k.begin(), k.end());
	polygon res;
	for (int i = 0; i < k.size(); i++) {
		while (res.size() > 1 && toLeft(k[i], (line) {res[res.size() - 2], res[res.size() - 1]}) <= 0) {
			res.pop();
		}
		res.push(k[i]);
	}
	...
	for (int i = k.size() - 2; i >= 0; i--) {
		while (res.size() > now && toLeft(k[i], (line) {res[res.size() - 2], res[res.size() - 1]}) <= 0) {
			res.pop();
		}
		res.push(k[i]);
	}
...
void rotatingCaliper(polygon &k, const auto &func) {
	// 必须保证k.convexCheck() = ture
	// 用投影写比较复杂
	if (k.size() == 1) {return;}  // 处理特殊情况
	if (k.size() == 2) {return;}
	auto area = [](const point & u, const point & v, const point & w) {return abs(cross((w - u) , (w - v)));};  // 还是要加上的，在右侧的时候是会算出来负数的

	int l = 1, r = 0;
	point dir = k[1] - k[0];
	line res = {k[0], k[0] + dir.acturn90()};
	while (toLeft(k[k.pre(r)], res) >= 0 && area(res[0], res[1], k[r]) <= area(res[0], res[1], k[k.pre(r)])) {
		r = k.pre(r);
	}
	for (int i = 0, j = 0; i < (int)k.size(); i++) {
		int nxti = k.nxt(i);
		while (area(k[i], k[nxti], k[j]) <= area(k[i], k[nxti], k[k.nxt(j)])) {
			j = k.nxt(j);
		}
		dir = k[nxti] - k[i];  // direction
		res = {k[nxti], k[nxti] + dir.acturn90()};
		while (toLeft(k[l], res) > 0 || (toLeft(k[k.nxt(l)], res) <= 0 && area(res[0], res[1], k[l]) <= area(res[0], res[1], k[k.nxt(l)]))) {
			l = k.nxt(l);
		}
		// cout << l << ": " << k.nxt(l) << " " << area(res[0], res[1], k[l]) << " " << area(res[0], res[1], k[k.nxt(l)]) << endl;
		res = {k[i], k[i] + dir.acturn90()};
		while (toLeft(k[r], res) < 0 || (toLeft(k[k.nxt(r)], res) >= 0 && area(res[0], res[1], k[r]) <= area(res[0], res[1], k[k.nxt(r)]))) {
			r = k.nxt(r);
		}
		// std::cout << i << " " << nxti << " " << j << " " << l << " " << r << std::endl;
		func(k[i], k[nxti], k[j], k[l], k[r]);
	}
}

db minAreaRectangleCover(polygon & k) {
	db ans = 1e15;
	// auto应该是一个地址值
	auto func = [&](const point & u, const point & v, const point & w1, const point & w2, const point & w3) {
		line res = {u, v};
		db lng = (projection(w2, res) - projection(w3, res)).abs();
		db wid = (w1 - projection(w1, res)).abs();
		ans = std::min(ans, lng * wid);
		std::cout << lng*wid << std::endl;
	};
	rotatingCaliper(k, func);
	return ans;
}
...
int main() {
	...
	polygon ans = convexHull(res);
	for (int i = 0; i < ans.size(); i++) {
		cout << ans[i].x << " " << ans[i].y << std::endl;
	}
	printf("%lf\n", minAreaRectangleCover(ans));
	return 0;
}