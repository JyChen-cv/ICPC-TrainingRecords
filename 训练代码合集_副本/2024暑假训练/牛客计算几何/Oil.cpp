...
#define fi first
#define se second
#define pb(x) push_back(x) // 调用pb里面只能含有一个参数，(point){...}会报错
#define P0 (point){0,0}
...
void argPointSort1(vector<point> &k) {
	// 1e9数据范围请开long double,精度很高，速度一般,常数比较大（如果能通过对称转换为半平面问题常数会得到优化）
...
void argPointSort2(vector<point> &k1, point k2) {
	int num = k1.size(); for (int i = 0; i < num; i++) k1[i] = (point) {k1[i].x - k2.x, k2.y - k1[i].y};
	argPointSort1(k1); for (int i = 0; i < num; i++) k1[i] = (point) {k1[i].x + k2.x, k2.y - k1[i].y};
}

const int N = 2000;
vector<point> mapp, mapp2;
db val[N + 2];
bool tag[N + 2], tt[N + 2];
...
int main() {
	...
	for (int i = 1; i <= n; i++) {
		...
		mapp.push_back((point) {a, c, i});
		mapp.push_back((point) {b, c, i});
		mapp2.push_back((point) {a, c, i});
		mapp2.push_back((point) {b, c, i});
		val[i] = std::fabs(b - a);
	}
	db ans = 0;
	for (int i = 0; i < (int)mapp.size(); i++) {
		for (int j = 0; j < (int)mapp.size(); j++)
			if (mapp[j].y < mapp2[i].y || (mapp[j].y == mapp2[i].y && mapp[j].x < mapp2[i].x)) tt[j] = 1;
			else tt[j] = 0;
		for (int j = 0; j < (int)mapp.size(); j++) if (tt[j]) mapp[j] = mapp2[i] * 2 - mapp[j];
		argPointSort2(mapp, mapp2[i]);

		db res = 0;
		memset(tag, 0, sizeof (tag));
		res += val[mapp2[i].fa];
		for (int j = 0; j < (int)mapp.size(); j++) {
			if (mapp[j].fa == mapp2[i].fa) continue;
			int t = j;
			while (t + 1  < (int)mapp.size() && onSegement(mapp[t + 1], (line) {mapp2[i], mapp[j]})) t++;
			for (int k = j; k <= t; k++) if (tag[mapp[k].fa] == 0) {tag[mapp[k].fa] = 1; res += val[mapp[k].fa];}
			ans = std::max(res, ans);
			for (int k = j; k <= t; k++) if (tag[mapp[k].fa]) {res -= val[mapp[k].fa];}
			j = t;
		}
		for (int j = 0; j < (int)mapp.size(); j++) if (tt[j]) mapp[j] = mapp2[i] * 2 - mapp[j];
		std::cout << i << std::endl;
		for (int j = 0; j < (int)mapp.size(); j++) mapp[j].print();
	}
	printf("%.11lf\n", ans);
	return 0;
}