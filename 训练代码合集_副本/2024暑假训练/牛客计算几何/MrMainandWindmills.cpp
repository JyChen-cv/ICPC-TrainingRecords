...
int main() {
	...
	ini.scan();
	int tag = 0;
	if (ini[1] < ini[0]) tag = 1;
	for (int i = 1; i <= n; i++) mapp[i].scan();
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= n; j++) {
			if (i == j) continue;
			if (lineSegIntersectionCheck((line) {mapp[i], mapp[j]}, ini)) {
				t[i].pb(intersection((line) {mapp[i], mapp[j]}, ini));
			}
		}
	...
	while (m--) {
		int a, b;
		std::cin >> a >> b;
		if (t[a].size() < b) std::cout << -1 << std::endl;
		else {
			if (tag == 0)t[a][b - 1].print();
			else t[a][t[a].size() - b].print();
		}
	}
