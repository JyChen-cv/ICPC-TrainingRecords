...
int main() {
	...
	while (t--) {
		...
		point p, v;
		p.scan(), v.scan();
		int k, tag = 0, tag1 = 0, tag2 = 0, tag3 = 0;
		std::cin >> k;
		tag = k % 2;
		k = k / 2;
		point res, l1, l2, tri1, tri2, tri3;
		l1 = (point) {0, l};
		l2 = (point) {l, 0};
		tri1 = l2 / 2 * (-1);
		tri2 = l2 / 2;
		tri3 = l1 / 2 * sqrt(3);
		tag1 = lineSegIntersectionCheck((line) {p, p + v}, (line) {tri1, tri2});
		tag2 = lineSegIntersectionCheck((line) {p, p + v}, (line) {tri2, tri3});
		tag3 = lineSegIntersectionCheck((line) {p, p + v}, (line) {tri3, tri1});
		if (tag2 && tag3) {
			if ()
				if (tag) {
					res = intersection((line) {p, p + v}, (line) {l2 / 2 + l2*k, l1*(k * 2 + 1) / 2 * sqrt(3)});
					p.print(), (p + v).print(), (l2 / 2 + l2 * k).print(), (l1 * (k * 2 + 1) / 2 * sqrt(3)).print();
				}
				else res = intersection((line) {p, p + v}, (line) {l2*k - l2 / 2, l1*(k * 2 - 1) / 2 * sqrt(3)*(-1)});
		}
		else if (tag1 && tag2) {
