...
std::vector<std::pair<int, std::pair<int, int>>> ls;
void solve() {
	std::cin >> n >> m;
	for (int i = 1; i <= m; i++) {
		int a, b, c;
	...
	for (auto i : ls) {
		...
		if (d1[a].first > c || d2[b].first > c) continue;
		std::cout << a << " " << b << " " << c << " " << ans << std::endl;
		ans = std::min(ans, c + std::max(d1[a].first, d2[b].first));
	}
